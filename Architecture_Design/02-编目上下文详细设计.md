...
**领域层 (domain/)**:
- 实体/聚合根: Book, Author, Category, Publisher, BookAuthor
- 值对象: ISBN
- 枚举: AuthorRole, BookStatus
- 领域事件: 4 个 — BookCreatedEvent, BookDeletedEvent, BookPublishedEvent, BookUpdatedEvent
- 领域服务: AuthorManagementService, BookManagementService, CategoryManagementService, ISBNValidationService, PublisherManagementService
- 仓储接口: AuthorRepository, BookRepository, CategoryRepository, CustomBookRepository, PublisherRepository
- 领域异常: 9 个
...
- Kafka 发布者: CatalogDomainEventPublisher（topic: library.domain-events）
- Kafka 消费者: 无

**接口层 (interfaces/)**:
- REST 控制器: AuthorController, BookController, CategoryController, PublisherController
...
实现与设计文档基本一致。主要差异：
- 实际实现中事件发布采用了 `CatalogDomainEventPublisher` 封装类，而非直接使用 `DomainEventPublisher` 接口
- 控制器拆分为 4 个独立控制器（AuthorController, BookController, CategoryController, PublisherController），设计文档中仅展示了 BookController
- 添加了 CustomBookRepository 用于复杂查询，设计中未明确提及
