...
| 模块 | main Java 文件 | 领域事件 | 领域异常 | Kafka 消费者 | Kafka 发布者 | 单元测试 | BDD Steps | Feature 文件 |
|------|---------------|---------|---------|-------------|------------|---------|-----------|-------------|
| library-shared | 23 | 1 (基类) | 0 | 0 | 1 (基类) | 6 | 0 | 0 |
| library-catalog | 49 | 4 | 9 | 0 | 1 | 22 | 5 | 4 |
| library-inventory | 46 | 8 | 7 | 2 | 1 | 9 | 4 | 5 |
| library-circulation | 48 | 14 | 6 | 1 | 1 | 7 | 1 | 2 |
| library-patron | 41 | 6 | 5 | 2 | 1 | 7 | 6 | 8 |
| library-payment | 37 | 6 | 3 | 1 | 1 | 6 | 2 | 2 |
| library-analytics | 33 | 4 | 4 | 2 | 1 | 7 | 3 | 3 |
| library-notification | 41 | 4 | 3 | 4 | 0 | 7 | 7 | 6 |
| library-e2e-test | 0 | 0 | 0 | 0 | 0 | 8 | 0 | 0 |
| library-integration-test | 0 | 0 | 0 | 0 | 0 | 0 | 8 | 7 |
| **合计** | **318** | **47** | **37** | **12** | **7** | **79** | **36** | **37** |

### 11.2 实际 Kafka 事件流拓扑

```mermaid
graph LR
    CATALOG["📚 Catalog"] -->|"BookCreatedEvent"| INVENTORY["📦 Inventory"]
    CATALOG -->|"BookCreatedEvent"| ANALYTICS["📊 Analytics"]
    
    CIRCULATION["🔄 Circulation"] -->|"BookBorrowedEvent"| PATRON["👤 Patron"]
    CIRCULATION -->|"BookBorrowedEvent"| INVENTORY
    CIRCULATION -->|"BookBorrowedEvent"| NOTIFICATION["🔔 Notification"]
    CIRCULATION -->|"BookReturnedEvent"| PATRON
    CIRCULATION -->|"BookReturnedEvent"| INVENTORY
    CIRCULATION -->|"BookReturnedEvent"| NOTIFICATION
    CIRCULATION -->|"FineIncurredEvent"| PATRON
    CIRCULATION -->|"FineIncurredEvent"| PAYMENT["💰 Payment"]
    CIRCULATION -->|"FineIncurredEvent"| NOTIFICATION
    CIRCULATION -->|"HoldPlacedEvent"| NOTIFICATION
    CIRCULATION -->|"HoldFulfilledEvent"| NOTIFICATION
    
    PATRON -->|"PatronSuspendedEvent"| CIRCULATION
    PATRON -->|"PatronSuspendedEvent"| NOTIFICATION
    
    INVENTORY -->|"LowStockAlertEvent"| NOTIFICATION
    INVENTORY -->|"LowStockAlertEvent"| ANALYTICS
    
    PAYMENT -->|"PaymentCompletedEvent"| PATRON
    PAYMENT -->|"PaymentCompletedEvent"| NOTIFICATION
```

### 11.3 双发模式（Double Publishing）

所有 7 个生产者模块遵循统一的双发模式：

```mermaid
graph TB
    Service["领域服务<br/>e.g. BookManagementService"] --> Publisher["*DomainEventPublisher"]
    Publisher -->|"① 同步必成功"| Local["Spring ApplicationEventPublisher<br/>(本地事件)"]
    Publisher -->|"② 异步可失败"| Kafka["Kafka KafkaTemplate<br/>(跨上下文)"]
    Kafka -->|"ObjectProvider&lt;KafkaTemplate&gt;"| Fallback["Kafka 不可用时优雅降级<br/>只发本地事件"]
```
...
- 各上下文 Publisher Bean 名称隔离（`@Component("catalogDomainEventPublisher")`）

### 11.4 DDD 分层实际文件分布

| 模块 | domain/ | application/ | infrastructure/ | interfaces/ |
|------|---------|-------------|-----------------|-------------|
| library-catalog | 31 | 9 | 5 | 5 |
| library-inventory | 25 | 13 | 3 | 3 |
| library-circulation | 29 | 11 | 4 | 4 |
| library-patron | 17 | 17 | 2 | 2 |
| library-payment | 18 | 13 | 2 | 2 |
| library-analytics | 16 | 10 | 2 | 2 |
| library-notification | 14 | 19 | 2 | 2 |

### 11.5 E2E 测试覆盖的跨上下文场景

| 编号 | 场景 | 事件流 | 验证点 |
|------|------|--------|--------|
| UC-1 | 借书 | Circulation → Patron + Notification | 贷款数+1, 通知创建 |
| UC-2 | 还书 | Circulation → Patron + Notification | 贷款数-1, 通知创建 |
| UC-3a | 预约排队 | Circulation → Notification | 通知创建 |
| UC-3b | 预约到馆 | Circulation → Notification | 通知创建 |
| UC-4 | 罚金产生 | Circulation → Patron + Payment + Notification | 罚款余额更新, 通知创建 |
| UC-5 | 支付完成 | Payment → Patron + Notification | 罚款清零, 通知创建 |
| UC-6 | 新书上架 | Catalog → Inventory | 库存记录创建 |
| UC-7 | 读者暂停 | Patron → Circulation + Notification | 通知创建 |
| UC-8 | 低库存预警 | Inventory → Notification | LIBRARIAN 通知创建 |
