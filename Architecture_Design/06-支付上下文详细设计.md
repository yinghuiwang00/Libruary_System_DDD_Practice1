...
**领域层 (domain/)**:
- 实体/聚合根: Payment, Refund
- 值对象: 无额外值对象
- 枚举: PaymentMethod, PaymentStatus, PaymentType, RefundStatus
- 领域事件: 6 个 — PaymentCancelledEvent, PaymentCompletedEvent, PaymentCreatedEvent, PaymentFailedEvent, RefundCompletedEvent, RefundRequestedEvent
- 领域服务: PaymentService
- 仓储接口: PaymentRepository, RefundRepository
- 领域异常: 3 个

**应用层 (application/)**:
- 应用服务: PaymentApplicationService
- 事件处理器: FineIncurredEventHandler

**基础设施层 (infrastructure/)**:
- Kafka 发布者: PaymentDomainEventPublisher（topic: library.payment.events）
- Kafka 消费者: CirculationEventConsumer（监听 library.circulation.events）

**接口层 (interfaces/)**:
- REST 控制器: PaymentController
...
实现与设计文档基本一致。主要差异：
- 设计文档中描述了 PaymentProcessingService、PaymentGatewayService、AlipayService、WechatPayService、UnionPayService 等多个领域服务，实际实现中合并为统一的 PaymentService
- 设计文档中详细描述了第三方支付网关集成（支付宝、微信、银联），实际实现中简化了网关集成部分
- 实际实现了 FineIncurredEventHandler 用于响应借阅上下文的罚金事件
- 实际实现了 CirculationEventConsumer 监听借阅上下文的事件
- 领域异常数量从设计中的多种细化异常简化为 3 个（DomainException, InvalidOperationException, PaymentNotFoundException）
