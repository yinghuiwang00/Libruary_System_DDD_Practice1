...
**领域层 (domain/)**:
- 实体/聚合根: Patron, BorrowingPrivilege
- 值对象: BorrowingPrivilege
- 枚举: MembershipStatus, PatronType
- 领域事件: 6 个 — PatronReactivatedEvent, PatronRegisteredEvent, PatronSuspendedEvent, PatronTerminatedEvent, PatronTypeChangedEvent, PatronUpdatedEvent
- 领域服务: PatronManagementService
- 仓储接口: PatronRepository
- 领域异常: 5 个

**应用层 (application/)**:
- 应用服务: PatronApplicationService
- 事件处理器: BookBorrowedEventHandler, BookReturnedEventHandler, FineIncurredEventHandler, PaymentCompletedEventHandler

**基础设施层 (infrastructure/)**:
- Kafka 发布者: PatronDomainEventPublisher（topic: library.patron.events）
- Kafka 消费者: CirculationEventConsumer（监听 library.circulation.events）、PaymentEventConsumer（监听 library.payment.events）

**接口层 (interfaces/)**:
- REST 控制器: PatronController
...
实现与设计文档基本一致。主要差异：
- 设计文档中描述了 PatronAccount、MembershipHistory 等复杂实体，实际实现中简化了会员账户和历史记录的管理
- 实际实现了 4 个事件处理器用于响应借阅、归还、罚金和支付事件，设计中未明确列出这些处理器
- 实际实现了 2 个 Kafka 消费者（CirculationEventConsumer 和 PaymentEventConsumer），用于监听借阅和支付上下文的事件
- BDD 测试覆盖较为丰富（8 个 feature 文件），是所有上下文中最多的
