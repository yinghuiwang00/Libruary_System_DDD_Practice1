...
@Component("patronFineIncurredEventHandler")
public class FineIncurredEventHandler {
    ...
    public FineIncurredEventHandler(PatronRepository patronRepository) { this.patronRepository = patronRepository; }

    @Transactional
    public void handle(JsonNode event) {
        String id = event.get("patronId").get("value").asText();
        BigDecimal amount = new BigDecimal(event.get("amount").asText());
        log.info("Handling FineIncurredEvent for patron: {}, amount: {}", id, amount);
        try {
        ...
        } catch (Exception e) { log.error("Failed to add fine for patron {}: {}", id, e.getMessage(), e); }
    }
}
