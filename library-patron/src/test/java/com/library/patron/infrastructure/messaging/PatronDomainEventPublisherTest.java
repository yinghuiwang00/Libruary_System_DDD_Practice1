...
class PatronDomainEventPublisherTest {
    ...
    private PatronDomainEventPublisher publisher;

    @BeforeEach
    void setUp() {
        when(environment.getProperty("spring.kafka.topic.domain-events", "library.patron.events")).thenReturn("library.patron.events");
    }
    ...
    void shouldPublishBothLocallyAndKafka() {
        ...
        publisher = new PatronDomainEventPublisher(localPublisher, kafkaProvider, environment);
        ...
        verify(kafkaTemplate).send("library.patron.events", "test-id", event);
    }
