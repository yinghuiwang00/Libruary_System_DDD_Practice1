...
class CirculationDomainEventPublisherTest {
    ...
    private CirculationDomainEventPublisher publisher;

    @BeforeEach
    void setUp() {
        when(environment.getProperty("spring.kafka.topic.domain-events", "library.circulation.events")).thenReturn("library.circulation.events");
    }
    ...
    void shouldPublishBothLocallyAndKafka() {
        ...
        publisher = new CirculationDomainEventPublisher(localPublisher, kafkaProvider, environment);
        ...
        verify(kafkaTemplate).send("library.circulation.events", "test-id", event);
    }
