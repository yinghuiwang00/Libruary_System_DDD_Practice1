...
class CatalogDomainEventPublisherTest {
    ...
    private CatalogDomainEventPublisher publisher;

    @BeforeEach
    void setUp() {
        when(environment.getProperty("spring.kafka.topic.domain-events", "library.domain-events")).thenReturn("library.catalog.events");
    }
    ...
    void shouldPublishBothLocallyAndKafka() {
        ...
        publisher = new CatalogDomainEventPublisher(localPublisher, kafkaProvider, environment);
        ...
        verify(kafkaTemplate).send("library.catalog.events", "test-id", event);
    }
